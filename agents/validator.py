...
def validate_reschedule_time(new_date: date, new_time: time) -> dict:
    ...
    if new_datetime < now_local:
        return {"valid": False, "error": "Невозможно перенести на прошедшую дату/время. Выберите будущую дату."}

    if new_date > (now_local + timedelta(days=60)).date():
        return {"valid": False, "error": "Перенос возможен максимум на 60 дней вперёд."}
